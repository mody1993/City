...
function createBot(config) {
    ...
    // دالة الإرسال الشاملة (تتوافق مع جميع إصدارات المكتبة)
    ...
    // الفحص الذكي وتحديث الميقاتي ديناميكياً
    ...
    client.on('ready', async () => {
        ...
        // 1️⃣ رحلة التشغيل الأولي
        await sendBoxCommand();

        // إطلاق الدورات المتوازية
        mainActionLoop(); 
        openBoxLoop(); 
        checkLoop();

        // 5️⃣ مرحلة الإغلاق النهائي (بعد 5 ساعات و 58 دقيقة)
        setTimeout(async () => {
            originalLog(`🛑 [${botName}] انتهى الوقت المسموح. جاري الإيقاف.`);
